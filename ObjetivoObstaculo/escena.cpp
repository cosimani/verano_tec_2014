...
//    this->setFixedSize(320, 240);
    ...
    // Uno o el otro. Ver el metodo slot_detectarOjos() para notar que se activa uno o el otro
    deteccionOjosActivo = false;
...
//        this->imageCamara = Util::getUtil()->IplImage2QImage(&image);
        ...
        if (this->deteccionOjosActivo)  {
//            IplImage copy = image;
//            IplImage* new_image = &copy;

//            cara = Util::getUtil()->detectaCara(new_image);
//            centroCara = cara.getContorno().getCentro();
//            qDebug() << "centroCara.x()= " << centroCara.x();

//            actualizaServoSiCorresponde(centroCara.x() * 2, centroCara.y());
        }
    }
}
...
//    glClearColor(1, 1, 1, 0);
    ...
    glGenTextures(1 + cantidadTexturasComunes + cantidadTexturas3ds, idTextura);
...
//            cv::Size2i sizeVentanaOpenGL(RESOLUTION_W, RESOLUTION_H);
            ...
            cv::Size2i sizeEscena(365, 285);
            cv::Size2i sizeVentanaOpenGL(365, 285);
...
//                glRotated(270, 1, 0, 0);  // Hago esta rotacion para que corresponda x y z como estoy acostumbrado
    ...
    // Entonces aca podemos con un switch y segun el id que este al frente de la camara dibujamos distintos objetos 3D
                ...
                ThreeDWriter tdw(100, "Arial", -0.2f+0.030f, 0.020f, -0.009f, 0.3f, 0.02f, 0.0001f);

                switch(this->markers->at(m).id)  {

                case 791:
                    dibujarModel("tanqueOil");
                    break;

                case 64:  // Marcador que tiene que ser seguido

                    if (seguirMarcadorConServo)
                        actualizaServoSiCorresponde(this->markers->at(m).getCenter().x, this->markers->at(m).getCenter().y);
                ...
                case 1005:  // Folleto de Informatica
                    dibujarCaja("EXPOTRONICA_UBP", 0.040f);
                    break;

                case 977:  // Caja tec
                    glTranslated(-0.050f, 0, 0);
                    dibujarCaja("tec", 0.025f);
                    break;
...
//                    dibujarPapelParaEscribir(A6, 180);
                ...
                case 985:
                    ...
                    tdw.setIsMirroredWriting(true); //Para que se espeje el texto
                    tdw.writeText("Bienvenidos a Expotronica!!");
                ...
                case 428:
                    ...
                    tdw.setIsMirroredWriting(true); //Para que se espeje el texto
                    tdw.writeText("Plan de estudio actualizado");
                    break;
...
// El numeroTexturaComun es 1 para el primer archivo de imagen en la carpeta texturas, 2 para el segundo, etc
void Escena::dibujarCaja(QString nombreTexturaComun, float sizeLadoCaja)  {
...
void Escena::actualizaServoSiCorresponde(int xActualMarcador, int yActualMarcador)  {

    if (xActualMarcador > 0 && xActualMarcador < RESOLUTION_W)  {  // Si cae fuera de esto es porque no se detecto una cara en la imagen

        if (xActualMarcador < RESOLUTION_W/2 - 30)  {  // Dejamos una franja de 60 (30+30) pixeles para que la camara considere que estamos centrados
...
//            serial.write(pc1, 1);
        ...
        else  if (xActualMarcador > RESOLUTION_W/2 + 30)  {
            valorServoHorizontal-=1;  // Este es el avance, la velocidad de acomodarse hacia el destino
...
//            serial.write(pc1, 1);
        ...
        if (yActualMarcador > 0 && yActualMarcador < RESOLUTION_H)  {  // Si cae fuera de esto es porque no se detecto una cara en la imagen
            ...
            if (yActualMarcador < RESOLUTION_H/2 - 30)  {  // Dejamos una franja de 60 (30+30) pixeles para que la camara considere que estamos centrados
            ...
            else  if (yActualMarcador > RESOLUTION_H/2 + 30)  {
                valorServoVertical-=1;  // Este es el avance, la velocidad de acomodarse hacia el destino
...
void Escena::slot_detectarOjos()  {
    this->setFocus();
    (this->deteccionOjosActivo==true) ? this->deteccionOjosActivo=false : this->deteccionOjosActivo=true;
    (this->seguirMarcadorConServo ==true) ? this->seguirMarcadorConServo=false : this->seguirMarcadorConServo=true;
}
