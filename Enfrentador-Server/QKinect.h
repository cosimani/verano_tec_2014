...
#define         FREENECT_FRAME_W   320
#define         FREENECT_FRAME_H   240
...
#define         FREENECT_FRAME_RGB_W   640
#define         FREENECT_FRAME_RGB_H   480

#define         FREENECT_FRAME_DEPTH_W   320
#define         FREENECT_FRAME_DEPTH_H   240

#define         FREENECT_FRAME_RGB_PIX   (FREENECT_FRAME_RGB_H*FREENECT_FRAME_RGB_W) // width*height pixels in the image
#define         FREENECT_VIDEO_RGB_SIZE   (FREENECT_FRAME_RGB_PIX*3) //3 bytes per pixel

#define         FREENECT_FRAME_DEPTH_PIX   (FREENECT_FRAME_DEPTH_H*FREENECT_FRAME_DEPTH_W) // width*height pixels in the image
#define         FREENECT_VIDEO_DEPTH_SIZE   (FREENECT_FRAME_DEPTH_PIX*3) //3 bytes per pixel
...
public :
    ...
    //----------------------------------------------------------------------------------------------------------------------
    /// @brief Publica con qDebug() la informacion de la camara de profundidad
    //----------------------------------------------------------------------------------------------------------------------
    void publicarCurrentDepthMode()  {
        freenect_frame_mode frameMode = freenect_get_current_depth_mode(m_dev);
        qDebug() << "Bits de datos por pixel = " << frameMode.data_bits_per_pixel;
        qDebug() << "Frames por segundo = " << frameMode.framerate;
        qDebug() << "ancho = " << frameMode.width << " - alto = " << frameMode.height;
        qDebug() << "Resolucion (0-320x240 1-640x480) = " << frameMode.resolution;
        qDebug() << "Formato (0-RGB ) = " << frameMode.video_format;
        qDebug() << "Formato depth (0-11BIT 1-10BIT) = " << frameMode.depth_format;
        qDebug() << "Bits de relleno = " << frameMode.padding_bits_per_pixel;
    }
...
private :
  ...
  QKinect();
	...
	/// @return EXIT_SUCCESS on success, EXIT_FAILURE om error
	//----------------------------------------------------------------------------------------------------------------------
	void init();
	//----------------------------------------------------------------------------------------------------------------------
