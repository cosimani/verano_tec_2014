...
Principal::Principal(QWidget *parent) : QWidget(parent),  ui(new Ui::Principal),
    lineal(0), angular(0), estadoKey(0), haceFaltaActualizarConCero(false), contPbProyector(0)
{
    ui->setupUi(this);

    clienteROS_image.vincularServicio("/TUBot/navegacion_por_imagenes");
    clienteROS_string.vincularServicio("/TUBot/control_por_string");
    ...
    connect(ui->cbImagen, SIGNAL(toggled(bool)), this, SLOT(timeout()));

    connect(ui->pbEnfrentar, SIGNAL(clicked()), this, SLOT(slot_enfrentar()));
    connect(ui->pbProyectar, SIGNAL(clicked()), this, SLOT(slot_proyectar()));
...
void Principal::keyPressEvent(QKeyEvent *e)  {
    ...
    case Qt::Key_Right:
        ...
        break;
    case Qt::Key_Q:
        clienteROS_string.enviarMensaje("ServoKinect", 1);
        break;
    case Qt::Key_A:
        clienteROS_string.enviarMensaje("ServoKinect", -1);
        break;
...
void Principal::pedirImagen(int ancho, int alto)  {
    ...
    qDebug() << "Se recbio una imagen de " << imageMat.cols << imageMat.rows;
...
//    if (contPbProyector > 2)  {
//        contPbProyector=0;
//    }
    clienteROS_string.enviarMensaje("Proyector", 1);  // 1 para enfrentar
    qDebug()<<"Enfrentar";

//    qDebug()<<contPbProyector;
//    contPbProyector++;

}

void Principal::slot_proyectar()  {
    clienteROS_string.enviarMensaje("Proyector", 2);  // 2 para proyectar
    qDebug()<<"Proyectar";
}
