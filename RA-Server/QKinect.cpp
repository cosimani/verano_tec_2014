...
//	freenect_set_log_level(m_ctx, FREENECT_LOG_DEBUG);  //////// EVito tanto log, activar si se necesita
    freenect_set_log_level(m_ctx, FREENECT_LOG_FATAL);
...
//    m_bufferVideo.resize(640*480*3);
//	m_bufferDepthRaw.resize(FREENECT_FRAME_PIX);
//	m_bufferDepthRaw16.resize(FREENECT_FRAME_PIX);


    m_bufferDepth.resize(FREENECT_VIDEO_DEPTH_SIZE);
    m_bufferVideo.resize(FREENECT_VIDEO_RGB_SIZE);
    m_bufferDepthRaw.resize(FREENECT_FRAME_DEPTH_PIX);
    m_bufferDepthRaw16.resize(FREENECT_FRAME_DEPTH_PIX);



//    m_bufferDepth.resize(FREENECT_VIDEO_DEPTH_SIZE);
//	m_bufferVideo.resize(FREENECT_VIDEO_RGB_SIZE);
//    qDebug() << "FREENECT_VIDEO_RGB_SIZE" << FREENECT_VIDEO_RGB_SIZE;
//    m_bufferDepthRaw.resize(FREENECT_FRAME_DEPTH_PIX);
//    m_bufferDepthRaw16.resize(FREENECT_FRAME_DEPTH_PIX);
    ...
    qDebug() << FREENECT_VIDEO_RGB_SIZE << FREENECT_FRAME_PIX;
...
//    freenect_frame_mode frameMode = freenect_get_video_mode(0);
//    freenect_frame_mode frameMode = freenect_get_current_video_mode(m_dev);
    freenect_frame_mode frameMode = freenect_get_current_depth_mode(m_dev);

//    freenect_set_video_mode(m_dev, frameMode);
    qDebug() << frameMode.dummy;  // 0  // 0
    qDebug() << frameMode.data_bits_per_pixel;  // 24  // 24
    qDebug() << frameMode.framerate;  // 10 // 30
    qDebug() << frameMode.height;  // 1024  // 480
    qDebug() << frameMode.width;  // 1280  // 640
    qDebug() << frameMode.resolution;  // 2  // 1
...
//        FREENECT_RESOLUTION_DUMMY  = 2147483647, /**< Dummy value to force enum to be 32 bits wide */
    qDebug() << frameMode.video_format;  // 0  // 0
...
//        FREENECT_VIDEO_DUMMY           = 2147483647, /**< Dummy value to force enum to be 32 bits wide */
    qDebug() << frameMode.depth_format;  // 0  // 0
...
//        FREENECT_DEPTH_DUMMY        = 2147483647, /**< Dummy value to force enum to be 32 bits wide */
    ...
    qDebug() << frameMode.padding_bits_per_pixel;  // 0  // 0
    qDebug() << frameMode.is_valid;  // 1  // 1
/**< If 0, this freenect_frame_mode is invalid and does not describe a supported mode.
 *Otherwise, the frame_mode is valid. */


//    freenect_frame_mode videoDepthFormat = freenect_find_depth_mode(FREENECT_RESOLUTION_LOW, FREENECT_DEPTH_11BIT);
//    qDebug() << "EXITO set buffer depth = " << freenect_set_depth_buffer(m_dev, m_bufferDepthRaw16.data());
//    qDebug() << "EXITO set depth = " << freenect_set_depth_mode(m_dev, videoDepthFormat);
...
void QKinect::setVideoMode(int _mode)
{
	...
	switch(_mode)
	{
	  ...
	  */
        default :
            qDebug()<<"index out of bounds for video mode\n";
