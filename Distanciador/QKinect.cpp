...
////    freenect_frame_mode videoRgbFormat = freenect_find_video_mode(FREENECT_RESOLUTION_HIGH, FREENECT_VIDEO_RGB);
//    freenect_frame_mode videoRgbFormat = freenect_find_video_mode(FREENECT_RESOLUTION_MEDIUM, FREENECT_VIDEO_RGB);
//    qDebug() << "EXITO set buffer video = " << freenect_set_video_buffer(m_dev, m_bufferVideo.data());
//    qDebug() << "EXITO set video = " << freenect_set_video_mode(m_dev, videoRgbFormat);
...
void QKinect::grabDepth(void *_depth, uint32_t _timestamp)
{
    ...
    for( unsigned int i = 0 ; i < FREENECT_FRAME_DEPTH_PIX ; ++i)
	{
